import { assert, base64Decode } from "../util";
import { BbsVerifyProofRequest, BbsCreateProofRequest } from "../../../../lib";

function verifyProofWithAllMessagesRevealedFromSingleMessageSignature(
  wasm: any
) {
  const messages = ["KNK0ITRAF+NrGg=="];
  const bbsPublicKey = base64Decode(
    ""
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: bbsPublicKey,
    messages,
    nonce: "v3bb/Mz+JajUdiM2URfZYcPuqxw=",
  };

  const result = wasm.verifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function verifyProofWithAllMessagesRevealedFromMultiMessageSignature(
  wasm: any
) {
  const messages = ["BVB6lAn912sz9Q==", "b45VqRkIo5R5Zw==", "yPqox0TIKS6vCA=="];
  const bbsPublicKey = base64Decode(
    ""
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: bbsPublicKey,
    messages,
    nonce: "dVPpzuQtJVAZzAw73beWiXLtoT4=",
  };
  const result = wasm.verifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function verifyProofWithOneMessageRevealedFromMultiMessageSignature(wasm: any) {
  const messages = ["+FxEv3VLcNZ8sA==", "eI2RcRExnbP8hw==", "wll4zckqWAb0Kg=="];
  const bbsPublicKey = base64Decode(
    ""
  );
  const proof = base64Decode(
    ""
  );

  const revealedMessages = messages.slice(0, 1);

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: bbsPublicKey,
    messages: revealedMessages,
    nonce: "NoWZhtX+u1wWLtUfPMmku1FtU2I=",
  };
  const result = wasm.verifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function failVerifyProofWithBadNonce(wasm: any) {
  const messages = ["KNK0ITRAF+NrGg=="];
  const bbsPublicKey = base64Decode(
    ""
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: bbsPublicKey,
    messages,
    nonce: "bad",
  };
  const result = wasm.verifyProof(request);
  assert(result.verified === false, "Invalid proof verification didn't fail");
}

function failVerifyProofWithUnsignedMessage(wasm: any) {
  // Expects messages to be ["Message1", "Message2", "Message3", "Message4"];
  const messages = ["BadMessage1", "Message2", "Message3", "Message4"];
  const bbsPublicKey = base64Decode(
    ""
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: bbsPublicKey,
    messages,
    nonce: "0123456789",
  };
  const result = wasm.verifyProof(request);
  assert(result.verified === false, "Invalid proof verification didn't fail");
}

function failVerifyProofWithRevealedMessageIntendedToBeHidden(wasm: any) {
  const messages = ["Message1", "Message2", "Message3", "Message4"];
  const signature = base64Decode(
    "j46NB7z6EBzD6q8bwBfzNYmjab3LPVoU7swcxO4qukq+qx0TrJhmo1TAW5UpDIFWSdb5kgWLAda2giwW4GImPTl8yWwIBJksnfT7zD8nonsvVaJh15/YrQ/n5KlknD4OtLTquji9RJK1U/xWzERHtA=="
  );
  const bbsPublicKey = base64Decode(
    ""
  );
  const nonce = "0123456789";

  const proofRequest: BbsCreateProofRequest = {
    signature,
    publicKey: bbsPublicKey,
    messages,
    revealed: [0],
    nonce,
  };
  const proof = wasm.createProof(proofRequest);

  // This is different from the jest test, but is clearer
  // Message3 wasn't revealed when creating the proof, but is
  // present in the messages array
  let proofMessages = ["Message3"];
  let request = {
    proof,
    publicKey: bbsPublicKey,
    messageCount: 4,
    messages: proofMessages,
    nonce,
    revealed: [0],
  };

  let result = wasm.verifyProof(request);
  assert(result.verified === false, "Invalid proof verification didn't fail");

  proofMessages = ["Message1"];
  request = {
    proof,
    publicKey: bbsPublicKey,
    messageCount: 4,
    messages: proofMessages,
    nonce,
    revealed: [0],
  };

  result = wasm.verifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function blsVerifyProofWithAllMessagesRevealedFromSingleMessageSignature(
  wasm: any
) {
  const messages = ["KnYAbm0fw3mlUA=="];
  const blsPublicKey = base64Decode(
    "qJgttTOthlZHltz+c0PE07hx3worb/cy7QY5iwRegQ9BfwvGahdqCO9Q9xuOnF5nD/Tq6t8zm9z26EAFCiaEJnL5b50D1cHDgNxBUPEEae+4bUb3JRsHaxBdZWDOo3pb"
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: blsPublicKey,
    messages,
    nonce: "4OS3nji2HNReo3QPHrdlxjOf8gc=",
  };

  let result = wasm.blsVerifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function blsVerifyProofWithAllMessagesRevealedFromMultiMessageSignature(
  wasm: any
) {
  const messages = ["ODLpUKee6nyz7g==", "v2zteJajIyIh5Q==", "x64hA8TTn4gYXg=="];
  const blsPublicKey = base64Decode(
    "qJgttTOthlZHltz+c0PE07hx3worb/cy7QY5iwRegQ9BfwvGahdqCO9Q9xuOnF5nD/Tq6t8zm9z26EAFCiaEJnL5b50D1cHDgNxBUPEEae+4bUb3JRsHaxBdZWDOo3pb"
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: blsPublicKey,
    messages,
    nonce: "ujMevaaq2n7Cg3ZLzXktqT/WRgM=",
  };

  let result = wasm.blsVerifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function blsVerifyProofWithOneMessageRevealedFromMultiMessageSignature(
  wasm: any
) {
  const messages = ["8NhsJO/MKxO74A==", "0noLBcl29ASJ2w==", "eMPpY348vqGDNA=="];
  const blsPublicKey = base64Decode(
    "qJgttTOthlZHltz+c0PE07hx3worb/cy7QY5iwRegQ9BfwvGahdqCO9Q9xuOnF5nD/Tq6t8zm9z26EAFCiaEJnL5b50D1cHDgNxBUPEEae+4bUb3JRsHaxBdZWDOo3pb"
  );
  const proof = base64Decode(
    ""
  );

  const revealedMessages = messages.slice(0, 1);

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: blsPublicKey,
    messages: revealedMessages,
    nonce: "I03DvFXcpVdOPuOiyXgcBf4voAA=",
  };

  let result = wasm.blsVerifyProof(request);
  assert(result.verified === true, "Proof verification failed");
}

function failBlsVerifyProofWithBadNonce(wasm: any) {
  const messages = ["KnYAbm0fw3mlUA=="];
  const blsPublicKey = base64Decode(
    "q4OA1O/iLQyZkwil2XatmWC1nWVSml3rd9UKV8zfy0hKOuzxpq0dJZhbRCp3tvCIGOkpRfSXKbUU9ns93QUas8n16nv6voGvffPNezqhnYknkzlnp2CwXgsEuVMp01l5"
  );
  const proof = base64Decode(
    ""
  );

  const request: BbsVerifyProofRequest = {
    proof,
    publicKey: blsPublicKey,
    messages,
    nonce: "bad",
  };

  let result = wasm.blsVerifyProof(request);
  assert(result.verified === false, "Invalid proof verification didn't fail");
}

export default {
  verifyProofWithAllMessagesRevealedFromSingleMessageSignature,
  verifyProofWithAllMessagesRevealedFromMultiMessageSignature,
  verifyProofWithOneMessageRevealedFromMultiMessageSignature,
  failVerifyProofWithBadNonce,
  failVerifyProofWithUnsignedMessage,
  failVerifyProofWithRevealedMessageIntendedToBeHidden,
  blsVerifyProofWithAllMessagesRevealedFromSingleMessageSignature,
  blsVerifyProofWithAllMessagesRevealedFromMultiMessageSignature,
  blsVerifyProofWithOneMessageRevealedFromMultiMessageSignature,
  failBlsVerifyProofWithBadNonce,
};
